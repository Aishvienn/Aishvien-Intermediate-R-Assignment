...
collatz_steps <- function(n) {
  ...
  while (n > 2) {         
    ...
    steps <- steps + 1
    steps <- c(n, steps) # need to fix list, doesnt make sense
  }
...
#|My guess is that when you divide vectors of different lengths that are not a multiple of each other it results in a error
#|for example the length of clean is 8, and n is 3 which also means that 3 is not a mutiple of 8
#| also another issue is that the value of n is "4L", not sure if that is correct
